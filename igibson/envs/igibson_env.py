...
from igibson.tasks.audiogoal_nav_task import AudioGoalNavTask
    ...
    def get_state(self):
        ...
        if "vision" in self.sensors:
            ...
            if len(self.config["VIDEO_OPTION"])!=0:
                ...
                state["depth"] = skimage.measure.block_reduce(state["depth"], (10,10,1), np.mean)
                if "rgb" in state.keys():
                    state['rgb_video'] = state['rgb']
                    state["rgb"] = skimage.measure.block_reduce(state["rgb"], (10,10,1), np.mean)
